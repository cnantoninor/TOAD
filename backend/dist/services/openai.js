...
class OpenAIService {
    static async generateResponse(messages, customInstructions, correlationId) {
        ...
        try {
            ...
            logger_1.Logger.logAiResponse(correlationId || 'unknown', correlationId || 'unknown', response.length, duration);
            return response;
        }
        catch (error) {
            const duration = Date.now() - startTime;
            logger_1.Logger.error('OpenAI API error', error, { correlationId, duration });
    ...
    static async summarizeConversation(messages, correlationId) {
        try {
            ...
            try {
                ...
                logger_1.Logger.error('Error parsing conversation summary', parseError, { correlationId });
                // Fallback summary
                return {
            ...
            logger_1.Logger.error('Error generating conversation summary', error, { correlationId });
            // Fallback summary
            return {
                keyPoints: ['Conversation in progress'],
                currentPhase: 1,
